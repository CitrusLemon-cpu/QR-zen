...
class Widget4x1Provider : AppWidgetProvider() {
    override fun onUpdate(context: Context, appWidgetManager: AppWidgetManager, appWidgetIds: IntArray) {
        ...
        for (id in appWidgetIds) {
            val v = RemoteViews(context.packageName, R.layout.widget_4x1)
            v.setTextViewText(R.id.tvStatus, status); v.setTextViewText(R.id.tvNextBlock, active.firstOrNull()?.title ?: "—"); v.setOnClickPendingIntent(R.id.root, pi)
