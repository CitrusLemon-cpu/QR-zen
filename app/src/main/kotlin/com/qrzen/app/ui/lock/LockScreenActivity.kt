...
class LockScreenActivity : AppCompatActivity() {
    ...
    private fun startQrScanner() {
        ...
        val intent = com.king.zxing.CaptureActivity.getLaunchIntent(this)
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == REQ_QR_SCAN && resultCode == RESULT_OK) {
            val scannedValue = com.king.zxing.util.CodeUtils.parseResult(data)
    ...
    private fun showPauseDurationSheet(block: AppBlock) {
        ...
        val sheetBinding = BottomSheetPauseDurationBinding.inflate(LayoutInflater.from(this))
        sheet.setContentView(sheetBinding.root)

        sheetBinding.btn15min.setOnClickListener { applyPause(block, 15 * 60_000L); sheet.dismiss() }
        sheetBinding.btn30min.setOnClickListener { applyPause(block, 30 * 60_000L); sheet.dismiss() }
        sheetBinding.btn1hr.setOnClickListener  { applyPause(block, 60 * 60_000L); sheet.dismiss() }
        sheetBinding.btn2hr.setOnClickListener  { applyPause(block, 2 * 60 * 60_000L); sheet.dismiss() }
        sheetBinding.btnRestOfDay.setOnClickListener {
            applyPause(block, millisUntilMidnight())
            sheet.dismiss()
        }
        sheetBinding.btnIndefinitely.setOnClickListener {
            applyPause(block, Long.MAX_VALUE)
            sheet.dismiss()
        }
        sheet.show()
    }
